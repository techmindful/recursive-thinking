...
var $author$project$Types$Part = function (a) {
	return {$: 'Part', a: a};
};
var $author$project$Types$Python = {$: 'Python'};
...
var $author$project$Types$More = {$: 'More'};
...
var $author$project$Main$routeParser = $elm$url$Url$Parser$oneOf(
	_List_fromArray(
		[
			...
			$author$project$Types$Part,
			A2(
				$elm$url$Url$Parser$slash,
				$elm$url$Url$Parser$s('part'),
				$elm$url$Url$Parser$int)),
			A2(
			$elm$url$Url$Parser$map,
			$author$project$Types$More,
			$elm$url$Url$Parser$s('more'))
		]));
...
var $author$project$Main$getRoute = function (url) {
	return A2(
...
var $author$project$Consts$maxPage = 7;
...
var $author$project$Main$init = F3(
	function (_v0, url, navKey) {
		var allPartRoutes = A2(
			$elm$core$List$map,
			$author$project$Types$Part,
			A2($elm$core$List$range, 1, $author$project$Consts$maxPage));
		var allQuizIDs = A3(
			$elm_community$list_extra$List$Extra$lift2,
			$elm$core$Tuple$pair,
			allPartRoutes,
			A2($elm$core$List$range, 1, 10));
		var allQuizStatuses = A3(
			$elm_community$list_extra$List$Extra$lift2,
			$elm$core$Tuple$pair,
			allQuizIDs,
			$elm$core$List$singleton(
				{sel: $author$project$Types$QuizNoInput, sub: $author$project$Types$QuizNoInput}));
					...
					_Utils_update(
						model,
						{
							quizStatuses: A3(
								...
								quizID,
								function (_v3) {
									return $elm$core$Maybe$Just(quizStatus);
								},
								model.quizStatuses)
						}),
					$elm$core$Platform$Cmd$none);
...
var $author$project$Consts$errPara = function (errMsg) {
	return A2(
...
var $author$project$PageNavButtons$defaultPrevPageBtnStyle = _List_fromArray(
	[
		A2($mdgriffith$elm_ui$Element$paddingXY, 0, 20)
	]);
var $author$project$Quiz$errGetQuizStatusPara = $author$project$Consts$errPara('Error getting QuizStatus.');
...
var $author$project$PageNavButtons$style = _List_fromArray(
	[
		$mdgriffith$elm_ui$Element$padding(5),
		$mdgriffith$elm_ui$Element$Border$width(2),
		$mdgriffith$elm_ui$Element$Border$rounded(6)
	]);
var $author$project$PageNavButtons$style_Disabled = _Utils_ap(
	$author$project$PageNavButtons$style,
	_List_fromArray(
		[
			$mdgriffith$elm_ui$Element$Border$color($author$project$Consts$disabledColor)
		]));
var $author$project$PageNavButtons$disabledButton = function (labelStr) {
	return A2(
		$mdgriffith$elm_ui$Element$Input$button,
		$author$project$PageNavButtons$style_Disabled,
		{
			label: A2(
				...
				$mdgriffith$elm_ui$Element$text(labelStr)),
			onPress: $elm$core$Maybe$Nothing
		});
};
var $author$project$PageNavButtons$getPartNum = function (model) {
	var _v0 = model.route;
	if (_v0.$ === 'Part') {
		var p = _v0.a;
...
var $author$project$PageNavButtons$illPlacedButton = A2(
	$mdgriffith$elm_ui$Element$Input$button,
	$author$project$PageNavButtons$style_Disabled,
	{
		label: A2(
			...
			_List_fromArray(
				[
					...
					A4($mdgriffith$elm_ui$Element$rgba, 255, 0, 0, 128))
				]),
			$mdgriffith$elm_ui$Element$text('Error: An ill-placed button 😦')),
		onPress: $elm$core$Maybe$Nothing
	});
...
var $author$project$PageNavButtons$linkToPart = F2(
	function (n, labelStr) {
		return A2(
...
var $author$project$PageNavButtons$mkNextPageButton = function (model) {
	var partNum = $author$project$PageNavButtons$getPartNum(model);
	if (partNum.$ === 'Just') {
		var n = partNum.a;
		return (_Utils_cmp(n, $author$project$Consts$maxPage) < 0) ? A2($author$project$PageNavButtons$linkToPart, n + 1, 'Next Page') : $author$project$PageNavButtons$disabledButton('Next Page');
	} else {
		return $author$project$PageNavButtons$illPlacedButton;
	}
};
var $author$project$PageNavButtons$mkPrevPageButton = function (model) {
	var partNum = $author$project$PageNavButtons$getPartNum(model);
	if (partNum.$ === 'Just') {
		var n = partNum.a;
		return (n > 1) ? A2($author$project$PageNavButtons$linkToPart, n - 1, 'Previous Page') : $author$project$PageNavButtons$disabledButton('Previous Page');
	} else {
		return $author$project$PageNavButtons$illPlacedButton;
	}
};
var $author$project$Quiz$mkQuizSubmitButton = F3(
	function (quizID, qStatus, labelStr) {
		return A2(
			$mdgriffith$elm_ui$Element$Input$button,
			_List_fromArray(
				[
				...
				]),
			{
				...
				onPress: $elm$core$Maybe$Just(
					A2(
						$author$project$Types$QuizRecvInput,
						quizID,
...
var $author$project$Consts$plainPara = function (str) {
	return A2(
		$mdgriffith$elm_ui$Element$paragraph,
		_List_Nil,
		_List_fromArray(
...
var $author$project$Quiz$quizRadioStyle = _List_fromArray(
	[
		$mdgriffith$elm_ui$Element$padding(20),
		$mdgriffith$elm_ui$Element$spacing(10)
	]);
var $author$project$Quiz$quizCorrectColor = A3($mdgriffith$elm_ui$Element$rgb255, 0, 150, 0);
var $author$project$Quiz$quizPassColor = A3($mdgriffith$elm_ui$Element$rgb255, 0, 0, 255);
var $author$project$Quiz$quizWrongColor = A3($mdgriffith$elm_ui$Element$rgb255, 255, 0, 0);
var $author$project$Quiz$quizRespColor = F2(
	function (qStatus, correctOp) {
		return $mdgriffith$elm_ui$Element$Font$color(
			function () {
				var _v0 = qStatus.sub;
				switch (_v0.$) {
					case 'QuizSel':
						var userSelection = _v0.a;
						return _Utils_eq(userSelection, correctOp) ? $author$project$Quiz$quizCorrectColor : $author$project$Quiz$quizWrongColor;
					case 'QuizPass':
						return $author$project$Quiz$quizPassColor;
					default:
						return $author$project$Quiz$quizWrongColor;
				}
			}());
	});
...
var $author$project$P1$p1 = function (model) {
	var maybeQuizOneStatus = A2(
		$pzp1997$assoc_list$AssocList$get,
		_Utils_Tuple2(
			$author$project$Types$Part(1),
			1),
		model.quizStatuses);
	if (maybeQuizOneStatus.$ === 'Nothing') {
		return $author$project$Quiz$errGetQuizStatusPara;
	} else {
		var quizOneStatus = maybeQuizOneStatus.a;
		return A2(
			...
			_Utils_ap(
				_List_fromArray(
					[
						A2(
						$mdgriffith$elm_ui$Element$el,
						$author$project$PageNavButtons$defaultPrevPageBtnStyle,
						$author$project$PageNavButtons$mkPrevPageButton(model)),
						$author$project$Consts$plainPara('The simplest example to explain recursion is factorial. Here\'s the traditional, \"iterative\" definition of what a factorial is:'),
						A2(
						$mdgriffith$elm_ui$Element$paragraph,
						_List_fromArray(
							[
						...
						_List_fromArray(
							[
								$mdgriffith$elm_ui$Element$text('The factorial of a positive integer n, denoted by n! , is the product of all the integers between n and 1. For example the factorial of 4, which is 4! , is just 4 * 3 * 2 * 1 = 24.')
							])),
						$author$project$Consts$plainPara('To put it in a mathematical equation:'),
						A2(
						$mdgriffith$elm_ui$Element$paragraph,
						...
						$author$project$Consts$plainPara('Now that we know what a factorial is, we can actually define it with a new, recursive way. The recursive definition will define factorial with a factorial. Do you want to figure out which of the following equation is the correct one?'),
						A2(
						$mdgriffith$elm_ui$Element$Input$radio,
						$author$project$Quiz$quizRadioStyle,
						{
							label: A2(
							...
							onChange: function (option) {
								return A2(
									...
									_Utils_Tuple2(
										$author$project$Types$Part(1),
										1),
									{sel: option, sub: quizOneStatus.sub});
							},
						...
						A3(
						$author$project$Quiz$mkQuizSubmitButton,
						_Utils_Tuple2(
							$author$project$Types$Part(1),
							1),
						quizOneStatus,
						...
						_List_fromArray(
							[
								...
								A2($author$project$Quiz$quizRespColor, quizOneStatus, 2)
							]),
						_List_fromArray(
							[
								$mdgriffith$elm_ui$Element$text(
								function () {
									var _v1 = quizOneStatus.sub;
									switch (_v1.$) {
										case 'QuizSel':
											if (_v1.a === 2) {
				...
				((!_Utils_eq(
					quizOneStatus.sub,
					$author$project$Types$QuizSel(2))) && (!_Utils_eq(quizOneStatus.sub, $author$project$Types$QuizPass))) ? _List_fromArray(
					...
					[
						...
						_List_fromArray(
							[
								$author$project$Consts$plainPara('According to the traditional definition of factorial:'),
								A2(
								$mdgriffith$elm_ui$Element$paragraph,
								$author$project$Consts$mathExpStyle,
								_List_fromArray(
								...
								$author$project$Consts$plainPara('If we multiply by n at both sides, we get:'),
								A2(
								$mdgriffith$elm_ui$Element$paragraph,
								$author$project$Consts$mathExpStyle,
								_List_fromArray(
								...
								$author$project$Consts$plainPara('The right side is actually just n! .'),
								A2(
								$mdgriffith$elm_ui$Element$paragraph,
								_List_fromArray(
									[
								...
								_List_fromArray(
									[
										$mdgriffith$elm_ui$Element$text('So we conclude:')
									])),
								A2(
								$mdgriffith$elm_ui$Element$paragraph,
								$author$project$Consts$mathExpStyle,
...
var $author$project$Consts$paraSpacing = A2($mdgriffith$elm_ui$Element$spacingXY, 0, 15);
var $author$project$P2$p2 = function (model) {
	var quiz2_SubmitButton = function (qStatus) {
		return A3(
			$author$project$Quiz$mkQuizSubmitButton,
			_Utils_Tuple2(
				$author$project$Types$Part(2),
				2),
			qStatus,
			'Submit');
	};
	var quiz2_Resp = function (quizTwoStatus) {
		return A2(
			$mdgriffith$elm_ui$Element$textColumn,
			_List_fromArray(
				[
					A2($mdgriffith$elm_ui$Element$paddingXY, 0, 15),
					A2($author$project$Quiz$quizRespColor, quizTwoStatus, 3)
				]),
			_List_fromArray(
				[
					function () {
					var _v3 = quizTwoStatus.sub;
					_v3$4:
					while (true) {
						switch (_v3.$) {
							case 'QuizSel':
								switch (_v3.a) {
									case 3:
										return $mdgriffith$elm_ui$Element$text('That is correct!');
									case 1:
										return $author$project$Consts$plainPara('That won\'t do. If that\'s added to the definition, the computation will be left with no answer when it reaches 1! = 1 * 0! .');
									case 2:
										return A2(
											$mdgriffith$elm_ui$Element$textColumn,
											_List_Nil,
											_List_fromArray(
												[
													$author$project$Consts$plainPara('Not quite. If 0! = 0, then'),
													A2(
													$mdgriffith$elm_ui$Element$textColumn,
													_List_fromArray(
														[
													...
													_List_fromArray(
														[
															$author$project$Consts$plainPara('1! = 1 * 0! = 1 * 0 = 0'),
															$author$project$Consts$plainPara('2! = 2 * 1! = 2 * 0 = 0'),
															$author$project$Consts$plainPara('3! = 3 * 2! = 3 * 0 = 0'),
															$author$project$Consts$plainPara('4! = 4 * 3! = 4 * 0 = 0'),
															$author$project$Consts$plainPara('...'),
															$author$project$Consts$plainPara('Every factorial will equal to 0.')
														]))
												]));
									default:
										break _v3$4;
							...
							case 'QuizPass':
								return $mdgriffith$elm_ui$Element$text('No problem. Solution is on the next page.');
							default:
								break _v3$4;
						}
					}
	...
	var quiz2_RadioButtons = function (quizTwoStatus) {
		return A2(
			$mdgriffith$elm_ui$Element$Input$radio,
			$author$project$Quiz$quizRadioStyle,
			{
	...
	var quiz2 = function (quizTwoStatus) {
		return A2(
			$mdgriffith$elm_ui$Element$column,
			_List_Nil,
			_List_fromArray(
	...
	var quiz1_SubmitButton = function (qStatus) {
		return A3(
			$author$project$Quiz$mkQuizSubmitButton,
			_Utils_Tuple2(
				$author$project$Types$Part(2),
	...
	var quiz1_Resp = function (quizOneStatus) {
		return A2(
			$mdgriffith$elm_ui$Element$paragraph,
			_List_fromArray(
				[
					A2($mdgriffith$elm_ui$Element$paddingXY, 0, 15),
					A2($author$project$Quiz$quizRespColor, quizOneStatus, 1)
				]),
			_List_fromArray(
				[
					$mdgriffith$elm_ui$Element$text(
					function () {
						var _v2 = quizOneStatus.sub;
						switch (_v2.$) {
							case 'QuizSel':
								if (_v2.a === 1) {
									return 'That is correct!';
								} else {
									return 'Simplicity doesn\'t mean incapability. Quite often simplicity is actually preferred in math and computer science!';
								}
							case 'QuizPass':
								return 'No problem. Second option is correct. Here\'s why:';
							default:
								return '';
						}
					}())
	...
	var quiz1_RadioButtons = function (quizOneStatus) {
		return A2(
			$mdgriffith$elm_ui$Element$Input$radio,
			$author$project$Quiz$quizRadioStyle,
			{
	...
	var quiz1 = function (quizOneStatus) {
		return A2(
			$mdgriffith$elm_ui$Element$column,
			_List_Nil,
			_List_fromArray(
	...
	var preQuiz2 = A2(
		...
		_List_fromArray(
			[
				$author$project$Consts$plainPara('If the entirety of the definition is just'),
				A2(
				$mdgriffith$elm_ui$Element$paragraph,
				$author$project$Consts$mathExpStyle,
				_List_fromArray(
				...
				$author$project$Consts$plainPara('Then imagine if we try to compute 4! .'),
				A2(
				$mdgriffith$elm_ui$Element$textColumn,
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						$author$project$Consts$plainPara('4! = 4 * 3! . But what does 3! equal to?'),
						$author$project$Consts$plainPara('3! = 3 * 2! . But what does 2! equal to?'),
						$author$project$Consts$plainPara('2! = 2 * 1! . But what does 1! equal to?'),
						$author$project$Consts$plainPara('1! = 1 * 0! . But what does 0! equal to?'),
						$author$project$Consts$plainPara('0! = 0 * (-1)! . But what does...')
					])),
				A2(
				$mdgriffith$elm_ui$Element$textColumn,
				_List_fromArray(
				...
				_List_fromArray(
					[
						$author$project$Consts$plainPara('Such a definition will be useless, because it never terminates. It\'s also incorrect in some sense. We will be multiplying negative numbers. And at the 5th step shown above, a 0 shows up in the multiplication, which means the result of any factorial will just be 0.'),
						$author$project$Consts$plainPara('But we can fix it with just one addition to our definition. Do you want to figure out what it is?')
					]))
			]));
	var preQuiz1 = A2(
		$mdgriffith$elm_ui$Element$textColumn,
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$text('So if you ask me what\'s the factorial of n, I can just tell you it\'s n times the factorial of n - 1. This may be an interesting answer itself. But this definition has a fatal flaw. Do you want to figure out what it is?')
					]))
			]));
	var afterQuiz1 = function () {
		var maybeQuizTwoStatus = A2(
			$pzp1997$assoc_list$AssocList$get,
			_Utils_Tuple2(
				$author$project$Types$Part(2),
				2),
			model.quizStatuses);
		if (maybeQuizTwoStatus.$ === 'Nothing') {
			return $mdgriffith$elm_ui$Element$none;
		} else {
			var quizTwoStatus = maybeQuizTwoStatus.a;
			return A2(
				$mdgriffith$elm_ui$Element$column,
				_List_Nil,
				_List_fromArray(
					[
						...
						(_Utils_eq(
						quizTwoStatus.sub,
						$author$project$Types$QuizSel(3)) || _Utils_eq(quizTwoStatus.sub, $author$project$Types$QuizPass)) ? $author$project$PageNavButtons$mkNextPageButton(model) : $mdgriffith$elm_ui$Element$none
					]));
		}
	}();
	return A2(
			...
			function () {
				...
				if (maybeQuizOneStatus.$ === 'Nothing') {
					...
					return _List_fromArray(
						[
							quiz1(quizOneStatus),
							(_Utils_eq(quizOneStatus.sub, $author$project$Types$QuizPass) || _Utils_eq(
							quizOneStatus.sub,
							$author$project$Types$QuizSel(1))) ? afterQuiz1 : $mdgriffith$elm_ui$Element$none
						]);
				}
			}()));
};
var $author$project$P3$p3 = function (model) {
	...
	return A2(
		...
		_List_fromArray(
			[
				prevPageButton,
				$author$project$Consts$plainPara('Our definition will be complete if we simply add 1! = 1 into it:'),
				A2(
				$mdgriffith$elm_ui$Element$textColumn,
				_List_fromArray(
					[
						A2($mdgriffith$elm_ui$Element$paddingXY, 40, 36),
						A2($mdgriffith$elm_ui$Element$spacingXY, 0, 10),
						$mdgriffith$elm_ui$Element$Font$size(24)
					]),
				_List_fromArray(
					[
						$author$project$Consts$plainPara('1! = 1'),
						$author$project$Consts$plainPara('n! = n * (n - 1)!')
					])),
				$author$project$Consts$plainPara('When asked \"what is the factorial of n\", we can keep answering that \"the factorial of n equals to n times the factorial of (n - 1), until we reach \"what is the factorial of 1\". At that point, we can just answer that \"the factorial of 1 equals to 1\". This enables us to answer all the previous questions, in reverse order. Let\'s look at the example of 4! again:'),
				A2(
				$mdgriffith$elm_ui$Element$textColumn,
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						$author$project$Consts$plainPara('4! = 4 * 3! . But what is 3! then?'),
						$author$project$Consts$plainPara('3! = 3 * 2! . But what is 2! then?'),
						$author$project$Consts$plainPara('2! = 2 * 1! . But what is 1! then?'),
						$author$project$Consts$plainPara('1! = 1, by definition. Therefore,'),
						$author$project$Consts$plainPara('2! = 2 * 1! = 2 * 1 = 2. Therefore,'),
...
var $author$project$Quiz$dummyQuizStatus = {sel: $author$project$Types$QuizNoInput, sub: $author$project$Types$QuizNoInput};
var $author$project$Quiz$getQuizStatus = F2(
	function (model, quizID) {
		return A2(
			$elm$core$Maybe$withDefault,
			$author$project$Quiz$dummyQuizStatus,
			A2($pzp1997$assoc_list$AssocList$get, quizID, model.quizStatuses));
	});
...
var $author$project$Consts$multiMathExpStyle = _List_fromArray(
	[
		A2($mdgriffith$elm_ui$Element$paddingXY, 40, 36),
		A2($mdgriffith$elm_ui$Element$spacingXY, 0, 10),
		$mdgriffith$elm_ui$Element$Font$size(24)
...
var $author$project$P4$p4 = function (model) {
	var quizTwoStatus = A2(
		...
		model,
		_Utils_Tuple2(
			$author$project$Types$Part(4),
			2));
	var quizThreeStatus = A2(
		$author$project$Quiz$getQuizStatus,
		model,
		_Utils_Tuple2(
			$author$project$Types$Part(4),
			3));
	var quizOneStatus = A2(
		$author$project$Quiz$getQuizStatus,
		model,
		_Utils_Tuple2(
			$author$project$Types$Part(4),
			1));
	var quizFourStatus = A2(
		$author$project$Quiz$getQuizStatus,
		model,
		_Utils_Tuple2(
			$author$project$Types$Part(4),
			4));
	var quiz4_SubmitButton = function (qStatus) {
		return A3(
			$author$project$Quiz$mkQuizSubmitButton,
			_Utils_Tuple2(
				$author$project$Types$Part(4),
	...
	var quiz4_Resp = function (qStatus) {
		return A2(
			$mdgriffith$elm_ui$Element$paragraph,
			_List_fromArray(
				[
					A2($mdgriffith$elm_ui$Element$paddingXY, 0, 15),
					A2($author$project$Quiz$quizRespColor, qStatus, 2)
				]),
			_List_fromArray(
				[
					function () {
					...
					while (true) {
						switch (_v3.$) {
							case 'QuizSel':
								switch (_v3.a) {
									case 2:
										return $mdgriffith$elm_ui$Element$text('That\'s correct!');
									case 1:
										return $mdgriffith$elm_ui$Element$text('This option is close, but it\'s not making sense. Since we are on LeftSide, adding m means going through the middle segment, and reaching RightSide. If the result of min is l + f(LeftSide, rem), it\'d look like we are then teleporting back to LeftSide somehow.');
									case 3:
										return $mdgriffith$elm_ui$Element$text('Not quite. This option doesn\'t make sense. If the result of min is m + r, it means we will be going through the middle segment, and then the right segment. But then f(LeftSide, rem) is added, which means we are teleporting back to LeftSide.');
									default:
										break _v3$4;
								}
							case 'QuizPass':
								return $mdgriffith$elm_ui$Element$text('Don\'t worry. This sure is a tough one. With practice, recursive thinking will be a second nature.');
							default:
								break _v3$4;
						}
					}
	...
	var quiz4_RadioButtons = function (qStatus) {
		return A2(
			$mdgriffith$elm_ui$Element$Input$radio,
			$author$project$Quiz$quizRadioStyle,
			{
				...
				options: _List_fromArray(
					[
						...
						A2(
							...
							_List_fromArray(
								[
									$author$project$Consts$plainPara('m + min('),
									$mdgriffith$elm_ui$Element$text('    l + f(LeftSide, rem),'),
									$author$project$Consts$plainPara(''),
									$mdgriffith$elm_ui$Element$text('    r + f(RightSide, rem),'),
									$author$project$Consts$plainPara(')')
								]))),
						A2(
						$mdgriffith$elm_ui$Element$Input$option,
						...
						A2(
							...
							_List_fromArray(
								[
									$author$project$Consts$plainPara('min('),
									$mdgriffith$elm_ui$Element$text('    l + f(LeftSide, rem),'),
									$author$project$Consts$plainPara(''),
									$mdgriffith$elm_ui$Element$text('    m + r + f(RightSide, rem)'),
									$author$project$Consts$plainPara(')')
								]))),
						A2(
						$mdgriffith$elm_ui$Element$Input$option,
						$author$project$Types$QuizSel(3),
						$author$project$Consts$plainPara('min(l, m + r) + f(LeftSide, rem)')),
						A2(
						$mdgriffith$elm_ui$Element$Input$option,
						$author$project$Types$QuizPass,
						$author$project$Consts$plainPara('I\'ll pass.'))
					]),
				selected: $elm$core$Maybe$Just(qStatus.sel)
			});
	};
	var quiz4 = A2(
		$mdgriffith$elm_ui$Element$column,
	...
	var quiz3_SubmitButton = function (qStatus) {
		return A3(
			$author$project$Quiz$mkQuizSubmitButton,
			_Utils_Tuple2(
				$author$project$Types$Part(4),
	...
	var quiz3_Resp = function (qStatus) {
		return A2(
			$mdgriffith$elm_ui$Element$paragraph,
			_List_fromArray(
				[
					A2($mdgriffith$elm_ui$Element$paddingXY, 0, 15),
					A2($author$project$Quiz$quizRespColor, qStatus, 1)
				]),
			_List_fromArray(
				[
					function () {
							...
							case 'QuizSel':
								switch (_v2.a) {
									case 1:
										return $mdgriffith$elm_ui$Element$text('That is correct!');
									case 2:
										return $mdgriffith$elm_ui$Element$text('This one doesn\'t make sense. It\'s saying we are always going through the middle segment. Starting from the left side, it means we\'ll then end up on the right side. But then if \"whichever of the following two is quicker\" turns out to be \"1. Going through left segment to ...\", we\'d find ourselves having to teleport back to the left side!');
									default:
										break _v2$3;
								}
							case 'QuizPass':
								return $mdgriffith$elm_ui$Element$text('No problem. For now, know that the first option is correct.');
							default:
								break _v2$3;
						}
					}
	...
	var quiz3_RadioButtons = function (qStatus) {
		var optionStyle = $mdgriffith$elm_ui$Element$textColumn(
			_List_fromArray(
				[
				...
				options: _List_fromArray(
					[
						...
						optionStyle(
							_List_fromArray(
								[
									$author$project$Consts$plainPara('If we are at an intersection on the left side, then the quickest path from there is whichever of the following two is quicker:'),
									A2(
									$mdgriffith$elm_ui$Element$paragraph,
									_List_fromArray(
										[
									...
									_List_fromArray(
										[
											$mdgriffith$elm_ui$Element$text('1. Going through left segment to reach the next intersection, and then going through \"the quickest path from that intersection to reach London\".')
										])),
									A2(
									$mdgriffith$elm_ui$Element$paragraph,
									_List_fromArray(
									...
									_List_fromArray(
										[
											$mdgriffith$elm_ui$Element$text('2. Going through middle segment, then right segment, to reach the next intersection, and then going through \"the quickest path from that intersection to reach London\".')
										])),
									$author$project$Consts$plainPara('It\'s similar if we are on the right side.')
								]))),
						A2(
						$mdgriffith$elm_ui$Element$Input$option,
						$author$project$Types$QuizSel(2),
						optionStyle(
							_List_fromArray(
								[
									$author$project$Consts$plainPara('If we are at an intersection on the left side, then the quickest path from there is going through the middle segment first, and then going through whichever of the following two is quicker:'),
									A2(
									$mdgriffith$elm_ui$Element$paragraph,
									_List_fromArray(
										[
									...
									_List_fromArray(
										[
											$mdgriffith$elm_ui$Element$text('2. Going through right segment to reach the next intersection, and then going through \"the quickest path from that intersection to reach London\".')
										])),
									$author$project$Consts$plainPara('It\'s similar if we are on the right side.')
								]))),
						A2(
	...
	var quiz2_Resp = function (qStatus) {
		return A2(
			$mdgriffith$elm_ui$Element$paragraph,
			_List_fromArray(
				[
				...
				[
					...
					while (true) {
						if (_v1.$ === 'QuizSel') {
							switch (_v1.a) {
								case 1:
									return $author$project$Consts$plainPara('Don\'t be tricked here! Imagine if segment GJ takes 50 minutes, rather than just 10. Then it\'s much quicker to cross the middle street, then go forward on the other side, as it only takes 25 + 8 = 33 minutes.');
								case 2:
									return $author$project$Consts$plainPara('Precisely!');
								default:
									break _v1$2;
							}
						} else {
	...
	var quiz2_RadioButtons = function (qStatus) {
		return A2(
			$mdgriffith$elm_ui$Element$Input$radio,
			$author$project$Quiz$quizRadioStyle,
			{
				...
				options: _List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Element$Input$option,
						$author$project$Types$QuizSel(1),
						$author$project$Consts$plainPara('In either case, the destination is just straight ahead. No need to cross the middle street.')),
						A2(
						$mdgriffith$elm_ui$Element$Input$option,
						$author$project$Types$QuizSel(2),
						$author$project$Consts$plainPara('In either case, it takes more time to cross the middle street and go forward on the other side, than to go forward on the current side.'))
					]),
				selected: $elm$core$Maybe$Just(qStatus.sel)
			});
	};
	...
	var quiz1_Resp = function (qStatus) {
		return A2(
			$mdgriffith$elm_ui$Element$paragraph,
			_List_fromArray(
				[
						...
						case 'QuizSel':
							switch (_v0.a) {
								case 1:
									return $mdgriffith$elm_ui$Element$text('That is correct!');
								case 2:
									return $author$project$Consts$plainPara('Think again. It\'s not trivial to have this figured out. Remember that the base case is usually the most trivial (easiest) case. It\'s also usually the last step in the recursion.');
								default:
									return $author$project$Consts$plainPara('This doesn\'t need to be figured out. We can just solve the case where we start at A. If the final solution is a path like A->B->D->... We can just omit the A->B part. It takes 0 minute, and it\'s equivalent to saying the path starts at B.');
							}
						case 'QuizPass':
							return $author$project$Consts$plainPara('No problem. The first option is correct. Here\'s why:');
						default:
							return $mdgriffith$elm_ui$Element$none;
					}
				}()
	...
	var quiz1_RadioButtons = function (qStatus) {
		return A2(
			$mdgriffith$elm_ui$Element$Input$radio,
			$author$project$Quiz$quizRadioStyle,
			{
				...
				options: _List_fromArray(
					[
						...
						A2(
							...
							_List_fromArray(
								[
									$mdgriffith$elm_ui$Element$text('Figuring out how to decide between driving forward and reach London, versus driving across middle street, then forward and reach London, when you are at the last intersection (either G or H).')
								]))),
						A2(
						$mdgriffith$elm_ui$Element$Input$option,
						$author$project$Types$QuizSel(2),
						A2(
							...
							_List_fromArray(
								[
									$mdgriffith$elm_ui$Element$text('Figuring out how to decide between driving forward, versus driving across middle street, then forward, when you are at the starting point (either A or B).')
								]))),
						A2(
						$mdgriffith$elm_ui$Element$Input$option,
						$author$project$Types$QuizSel(3),
						A2(
							...
							_List_fromArray(
								[
									$mdgriffith$elm_ui$Element$text('Figuring out how to decide between starting at A, versus starting at B.')
								]))),
						A2(
						$mdgriffith$elm_ui$Element$Input$option,
						$author$project$Types$QuizPass,
	...
	var preQuiz4 = A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$author$project$Consts$plainPara('Now let\'s think of it this way: We\'re writing the solution of this problem as a function called f. If we tell f our current point, f should be able to tell us the least time needed to reach London from where we are.'),
						$author$project$Consts$plainPara('To give f the complete information regarding a point, we need to specify the side of that point, and the time lengths of the remaining segments ahead of that point. So point G can be described with \"LeftSide, [10, 25, 8]\", point F can be described with \"RightSide, [40, 20, 2, 10, 25, 8]\", and so on.'),
						$author$project$Consts$plainPara('Since we have already figured out the base case of the problem, let\'s write down the base case for f:')
					])),
				A2(
				$mdgriffith$elm_ui$Element$textColumn,
				_List_fromArray(
				...
				_List_fromArray(
					[
						$author$project$Consts$plainPara('f(LeftSide, [l, m, r]) = min(l, m + r)'),
						$author$project$Consts$plainPara('f(RightSide, [l, m, r]) = min(r, m + l)')
					])),
				A2(
				$mdgriffith$elm_ui$Element$textColumn,
				_List_fromArray(
				...
				_List_fromArray(
					[
						$author$project$Consts$plainPara('Let me explain: The base case is really the case where there are only 3 segments left. The segment on the left is denoted as l, the one in the middle as m, the one on the right as r. min(a, b) means \"the minimum between a and b\". From the previous quiz, if we are on LeftSide (like point G), the least time is the minimum between left segment l, versus the sum of the middle segment and the right segment. Similar reasoning for RightSide.'),
						$author$project$Consts$plainPara('Now let\'s tackle the more general, recursive case. This will enable us to solve the problem at any point on the map. Let\'s start at LeftSide. I\'m going to write the expression as:')
					])),
				A2(
				$mdgriffith$elm_ui$Element$paragraph,
				$author$project$Consts$multiMathExpStyle,
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$text('f(LeftSide, [l, m, r ||| rem])')
					])),
				A2(
				$mdgriffith$elm_ui$Element$textColumn,
				_List_fromArray(
				...
				_List_fromArray(
					[
						$author$project$Consts$plainPara('Where l, m, r are the left, middle, right segments right in front of us. And all of the remaining segments further ahead of us, are abbreviated as rem.'),
						$author$project$Consts$plainPara('What does it equal to?')
					]))
			]));
	var preQuiz3 = A2(
		$mdgriffith$elm_ui$Element$textColumn,
		...
		_List_fromArray(
			[
				$author$project$Consts$plainPara('Let\'s verbalize this strategy: At the last intersection, if we are on the left side, the quickest path is whichever of these two takes less time: Going through left segment, versus going through middle segment, then right segment.'),
				$author$project$Consts$plainPara('And it\'s similar if we are on the right side. The quickest path is whichever of these two takes less time: Going through right segment, versus going through middle segment, then left segment.'),
				$author$project$Consts$plainPara('Now let\'s tackle the more general, recursive case. With the solution of the base case at hand, can you answer the question \"At any intersection, what\'s the quickest path from there to reach London\"? Remember that in recursion, you can answer a question with a question!')
			]));
	var preQuiz2 = A2(
		$mdgriffith$elm_ui$Element$column,
		_List_Nil,
	...
	var postQuiz4 = A2(
		...
		_List_fromArray(
			[
				$mdgriffith$elm_ui$Element$text('postQuiz4')
			]));
	var postQuiz3 = A2(
		$mdgriffith$elm_ui$Element$column,
		_List_Nil,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$author$project$Consts$plainPara('And... That\'s it! That describes our entire strategy!'),
						$author$project$Consts$plainPara('For real. At any intersection, we have a definitive way to find out the quickest path. Even though it\'ll lead us to a new question, eventually we\'ll find ourselves reaching one of the last intersections (G or H). And from the base case, we know the quickest path from there. That in turn gives answer to the previous question, which gives answer to the previous question of the previous question, ... Eventually, the original question is answered.'),
						$author$project$Consts$plainPara('If you are not so convinced that a lazy, or even cheating way of problem-solving can actually work, then the following pages will go through the strategy step by step, to verify the recursion is correct.')
					])),
				A2(
				$mdgriffith$elm_ui$Element$el,
				_List_fromArray(
	...
	var postQuiz2 = A2(
		...
		_List_fromArray(
			[
				...
				(_Utils_eq(
				quizThreeStatus.sub,
				$author$project$Types$QuizSel(1)) || _Utils_eq(quizThreeStatus.sub, $author$project$Types$QuizPass)) ? postQuiz3 : $mdgriffith$elm_ui$Element$none
			]));
	var postQuiz1 = A2(
		$mdgriffith$elm_ui$Element$column,
		_List_Nil,
		_List_fromArray(
			[
				...
				_Utils_eq(
				quizTwoStatus.sub,
				$author$project$Types$QuizSel(2)) ? postQuiz2 : $mdgriffith$elm_ui$Element$none
			]));
	return A2(
		$mdgriffith$elm_ui$Element$column,
		_List_Nil,
...
var $author$project$P5$p5 = function (model) {
	var body = function () {
		...
		var qPara = $author$project$Consts$plainPara;
		var aParaEnd = function (str) {
			return A2(
				$mdgriffith$elm_ui$Element$paragraph,
				_List_fromArray(
					[
		...
		var aPara = $author$project$Consts$plainPara;
		return A2(
			$mdgriffith$elm_ui$Element$textColumn,
			_List_fromArray(
				[$author$project$Consts$paraSpacing]),
				...
				[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$mdgriffith$elm_ui$Element$text('Base case!')
								])),
					...
					_List_fromArray(
						[
							$author$project$Consts$plainPara('A: Since \"the quickest path starting from E\" is whichever of the following two paths takes less time:'),
							A2(
							$mdgriffith$elm_ui$Element$textColumn,
							vsOneTwoStyle,
							_List_fromArray(
								[
									$author$project$Consts$plainPara('1. EG followed by \"the quickest path starting from G\", which is actually EG + GJ and takes 40 + 10 = 50 minutes;'),
									$author$project$Consts$plainPara('2. EF + FH followed by \"the quickest path starting from H\", which is actually EF + FH + HK and takes 20 + 2 + 8 = 30 minutes.')
								])),
					...
					_List_fromArray(
						[
							$author$project$Consts$plainPara('A: Since \"the quickest path starting from F\" is whichever of the following two paths takes less time:'),
							A2(
							$mdgriffith$elm_ui$Element$textColumn,
							vsOneTwoStyle,
							_List_fromArray(
								[
									$author$project$Consts$plainPara('1. FH followed by \"the quickest path starting from H\", which is actually FH + HK and takes 2 + 8 = 10 minutes;'),
									$author$project$Consts$plainPara('2. EF + EG followed by \"the quickest path starting from G\", which is actually EF + EG + GJ and takes 20 + 40 + 10 = 70 minutes.')
								])),
					...
					_List_fromArray(
						[
							$author$project$Consts$plainPara('A: Since \"the quickest path starting from C\" is whichever of the following two paths takes less time:'),
							A2(
							$mdgriffith$elm_ui$Element$textColumn,
							vsOneTwoStyle,
							_List_fromArray(
								[
									$author$project$Consts$plainPara('1. CE followed by \"the quickest path starting from E\", which is actually CE + (EF + FH + HK) and takes 5 + 30 = 35 minutes;'),
									$author$project$Consts$plainPara('2. CD + DF followed by \"the quickest path starting from F\", which is actually CD + DF + (FH + HK) and takes 30 + 90 + 10 = 130 minutes.')
								])),
					...
					_List_fromArray(
						[
							$author$project$Consts$plainPara('A: Since \"the quickest path starting from D\" is whichever of the following two paths takes less time:'),
							A2(
							$mdgriffith$elm_ui$Element$textColumn,
							vsOneTwoStyle,
							_List_fromArray(
								[
									$author$project$Consts$plainPara('1. DF followed by \"the quickest path starting from F\", which is actually DF + (FH + HK) and takes 90 + 10 = 100 minutes;'),
									$author$project$Consts$plainPara('2. CD + CE followed by \"the quickest path starting from E\", which is actually CD + CE + (EF + FH + HK) and takes 30 + 5 + 30 = 65 minutes.')
								])),
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$mdgriffith$elm_ui$Element$text('Final answer!')
								])),
							$author$project$Consts$plainPara('A: Since \"the quickest path starting from A\" is whichever of the following two paths takes less time:'),
							A2(
							$mdgriffith$elm_ui$Element$textColumn,
							vsOneTwoStyle,
							_List_fromArray(
								[
									$author$project$Consts$plainPara('1. AC followed by \"the quickest path starting from C\", which is actually AC + (CE + EF + FH + HK) and takes 50 + 35 = 85 minutes;'),
									$author$project$Consts$plainPara('2. AB + BD followed by \"the quickest path starting from D\", which is actually AB + BD + (CD + CE + EF + FH + HK) and takes 0 + 10 + 65 = 75 minutes.')
								])),
							$author$project$Consts$plainPara('Therefore, \"the quickest path starting from A\" is AB + BD + CD + CE + EF + FH + HK. It takes 75 minutes.'),
							aPara('And this path is equivalent to omitting AB (which takes 0 minute), starting at B, and going through BD + CD + CE + EF + FH + HK.')
						])),
					$author$project$Consts$plainPara('There we have it! The quickest path from Heathrow to London is to start from B, and go through BD + CD + CE + EF + FH + HK. It takes 75 minutes.'),
					$author$project$Consts$plainPara('Recursion actually works!')
				]));
	}();
	return A2(
		$mdgriffith$elm_ui$Element$column,
		...
		_List_fromArray(
			[
				...
				A2(
				$mdgriffith$elm_ui$Element$el,
				$author$project$PageNavButtons$defaultNextPageBtnStyle,
				$author$project$PageNavButtons$mkNextPageButton(model))
			]));
};
	...
	var body = A2(
		...
		_List_fromArray(
			[
				...
				function () {
				var langSelButton = F2(
					function (lang, labelStr) {
						return A2(
							$mdgriffith$elm_ui$Element$Input$button,
							_List_fromArray(
								[
								...
								]),
							{
								label: $mdgriffith$elm_ui$Element$text(labelStr),
								onPress: $elm$core$Maybe$Just(
									$author$project$Types$SelectDemoCodeLang(lang))
							});
					});
				return A2(
					$mdgriffith$elm_ui$Element$row,
					...
					_List_fromArray(
						[
							A2(langSelButton, $author$project$Types$Python, 'Python'),
							A2(langSelButton, $author$project$Types$Haskell, 'Haskell'),
							function () {
							var _v0 = model.demoCodeLang;
							if (_v0.$ === 'Python') {
								return $mdgriffith$elm_ui$Element$none;
							} else {
								return A2(
									...
									$mdgriffith$elm_ui$Element$text('Much less code..'));
							}
						}()
						]));
			}(),
							...
							A2(
								...
								_List_fromArray(
									[
										$elm$html$Html$text(
										function () {
											var _v1 = model.demoCodeLang;
											if (_v1.$ === 'Python') {
												return $author$project$Consts$pythonCode;
											} else {
												return $author$project$Consts$haskellCode;
											}
										}())
									]))))
					])),
				...
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$text('Throw it into your compiler and see if it works!')
					]))
			]));
	return A2(
		$mdgriffith$elm_ui$Element$column,
...
var $author$project$P7$p7 = function (model) {
	var body = A2(
		$mdgriffith$elm_ui$Element$textColumn,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				$author$project$Consts$plainPara('That concludes the entirety of this explainer. I hope I didn\'t make it too dense. I hope you\'ve not only learned what recursion is, but also learned a new mindset of problem-solving. A new way of thinking, in general.'),
				A2(
				$mdgriffith$elm_ui$Element$paragraph,
				_List_Nil,
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$text('You can also check out '),
						A2(
						$mdgriffith$elm_ui$Element$link,
						_List_fromArray(
							[
						...
						{
							label: $mdgriffith$elm_ui$Element$text('More'),
							url: $author$project$Consts$moreUrlStr
						}),
						$mdgriffith$elm_ui$Element$text(' about recursion, functional programming, this website, and so on.')
					]))
			]));
	return A2(
		$mdgriffith$elm_ui$Element$column,
	...
	return $elm$core$Dict$fromList(
		_List_fromArray(
			[
				...
				_Utils_Tuple2(
				3,
				$author$project$P3$p3(model)),
				_Utils_Tuple2(
				4,
				$author$project$P4$p4(model)),
				_Utils_Tuple2(
				5,
				$author$project$P5$p5(model)),
				_Utils_Tuple2(
				6,
				$author$project$P6$p6(model)),
				_Utils_Tuple2(
				7,
		...
		[
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$text('Hi there, welcome! Here on this website, I\'m going to show you the concept of recursive thinking, a.k.a. recursion.')
						])),
					A2(
					$mdgriffith$elm_ui$Element$paragraph,
					_List_Nil,
					...
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$text('Let\'s dive into it!')
						]))
				])),
			A2(
			$mdgriffith$elm_ui$Element$el,
			...
			A2(
				...
				{
					label: $mdgriffith$elm_ui$Element$text('Begin'),
					url: '/part/1'
				}))
		]));
					...
					var banner = A2(
						...
						_List_fromArray(
							[
								...
								$mdgriffith$elm_ui$Element$text('Recursive')),
								A2(
								$mdgriffith$elm_ui$Element$el,
								_List_fromArray(
									[
								...
								$mdgriffith$elm_ui$Element$text('Thinking'))
							]));
					return A2(
						$mdgriffith$elm_ui$Element$column,
						_List_fromArray(
						...
						_Utils_ap(
							...
							function () {
								...
								switch (_v0.$) {
									...
									case 'More':
										return _List_fromArray(
											[$author$project$More$more]);
									default:
										return _List_fromArray(
											[
												$mdgriffith$elm_ui$Element$text('Route error')
											]);
								}
							}()));
				}())
